...
function myFunction() {
	...
	for (var i = 0; i < letters.length; i++) {
		...
		if (bin[i].length < Math.ceil(Math.log2(numLetters))) {
			bin[i] = "0" + bin[i];
			i--;
		}
	}
	...
	var huffSize = [];
	for (var i = 0; i < letters.length; i++) {
		var code = complete.find(letters[i], complete.traverseDF);
    	someString += letters[i] + ": " + code.encoded + "</br>";
    	huffSize[i] = code.encoded.length;
	}
	someString += "</br>So the string \"" + input + "\" becomes ";
	var huffEncoded = "";
	for (var i = 0; i < input.length; i++) {
		var code = complete.find(input[i], complete.traverseDF);
	...
	var binEncoded = "";
	for (var i = 0; i < input.length; i++) {
		var q = findIndex(letters, input[i]);
		var code = bin[q];
    	someString += code;
    	binEncoded += code;
	}
	control = 6;
	document.getElementById(divs[control]).innerHTML = words[control] + Math.ceil(Math.log2(numLetters)) + " bits/character:</p>" + "<p>" + someString + ".</p>";
...
function printAll(array, index) {
	...
	//console.log(maxI);
	document.getElementById(divs[index]).innerHTML = words[control] + "<svg width ='1250' height =" + (yScale*(maxI+1)) + " fill = black>" + prnt + "</svg>";
}
...
Tree.prototype.find = function(findData, traversal) {
    var blah = "42", 
    	callback = function(node) {
            if (node.data.charAt(0) == findData && node.children.length == 0) {
                blah = node;
            }
        };
...
Tree.prototype.print = function(x, y) {
	...
	var callback = function(node) {
            if (node.parent == null) {
            	...
            	//console.log(foo);
            	node.posX = node.parent.posX + 25*(j%2 ? -1 : 1)*Math.pow(scl, depth)/foo;
